...
def get_ai_requirement():
    ...
    headers = {"Authorization": authorization_token}
        ...
        # Realizar la primera petición para obtener datos del proyecto
    try:
        project_response = requests.get(
            f"http://localhost:8080/Project/{projectId}",
            headers=headers
        )
        ...
        return jsonify({
            "error": "Failed to get project data from backend.",
            "details": str(e)
        }), 500

    # Realizar la segunda petición para obtener los requerimientos del proyecto
    try:
        requirements_response = requests.get(
            f"http://localhost:8080/Project/{projectId}/Requirements",
            headers=headers
        )
        ...
        return jsonify({
            "error": "Failed to get requirements data from backend.",
            "details": str(e)
        }), 500
    ...
    data_as_string = json.dumps(combined_data)
